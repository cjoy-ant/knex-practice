-- BEGIN and COMMIT specify the start and end of a transaction
-- used around multiple commands to combine them into one transaction
-- if the first command works, but the second one fails, 
-- Postgres will undo the first command so we're not left with 
-- a "half-cooked state"
-- Great for seeding or alterting multiple tables
BEGIN;
